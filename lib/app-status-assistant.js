...
async function deliverAppStatusResult({ interaction, message, progressMessage, result, hubSession = null }) {
  const parts = [result.content, ...(result.extraMessages || [])].filter(Boolean);

  if (hubSession) {
    await hubSession.sendMain(parts[0]);
    for (const part of parts.slice(1)) {
      await hubSession.sendFollowUp(part);
    }
    return;
  }
