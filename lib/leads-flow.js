...
const { parseLeadsQuery } = require('./leads-query');
const { searchLeadsAcrossSpreadsheets } = require('./leads-search');
...
async function runLeadsLookupViaDrive(question) {
  ...
  if (query.mode !== 'file') {
    const search = await searchLeadsAcrossSpreadsheets(question);
  ...
  if (!question?.trim()) {
    return {
      content:
        ...
        + '`/leads question:6912345678` (αναζήτηση σε όλα τα Excel)\n'
...
async function handleLeadsInteraction(interaction, hubSession = null) {
  ...
  const query = parseLeadsQuery(question);
  const loadingMessage = query.mode === 'file'
    ? '⏳ Αναζήτηση αρχείων leads στο Google Drive…'
    : '⏳ Σάρωση όλων των Excel για την τιμή που έδωσες…';
  await session.sendLoading(loadingMessage);
  ...
  for (const extra of result.extraMessages || []) {
    await session.sendFollowUp(extra);
  }
}
