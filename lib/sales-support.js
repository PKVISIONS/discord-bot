...
function formatVendorSource(source) {
  const label = source.title || source.sourcePath;
  if (source.url) {
    return `- \`${source.sourcePath}\` — [Discord message](${source.url}) (${label})`;
  }
...
function buildEmployeeSourceBlock({
  ...
  const wikiSources = usedRetrieval
    ...
    : [];
  const vendorSources = usedRetrieval
    ? retrievalSources.filter((s) => s.namespace === 'vendor')
    : [];
  const otherSources = usedRetrieval
    ? retrievalSources.filter((s) => !['wiki', 'faq', 'vendor'].includes(s.namespace))
    : [];
  ...
  if (vendorSources.length) {
    lines.push('', '**Discord vendor files**');
    for (const source of vendorSources) {
      lines.push(formatVendorSource(source));
    }
  }
