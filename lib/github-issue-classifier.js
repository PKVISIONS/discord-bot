...
function resolveGitHubIssueTypeName(type) {
  const normalized = normalizeIssueType(type);
  if (!normalized) return null;
  const envKey = `GITHUB_ISSUE_TYPE_${normalized.toUpperCase()}`;
...
async function classifyGitHubIssueType({ title, description, apiKey = process.env.OPENAI_API_KEY }) {
  ...
  const userPrompt = [
    ...
    '',
    'Return JSON only:',
    '{ "type": "bug" | "feature" | "task", "confidence": "high" | "medium" | "low", "reason": "one short sentence in English" }',
    '',
    'Rules:',
