...
function lookbackStartIso(startKey) {
  // Inclusive start of the lookback window (UTC midnight of startKey).
  return `${startKey}T00:00:00Z`;
}

function isCommitInLookback(commit, lookbackKeys) {
  return lookbackKeys.includes(commit.dateKey);
}

function pickBranchesToScan(allBranches, defaultBranch, lookbackKeys, timeZone) {
  ...
  for (const branch of allBranches) {
    if (!branch?.name || !branch.date) continue;
    const key = formatDateKey(new Date(branch.date), timeZone);
    // Tip active in lookback, or tip slightly newer than lookback end (still scan).
    if (lookbackSet.has(key) || key >= lookbackKeys[lookbackKeys.length - 1]) {
      add(branch.name);
    }
  }
...
async function fetchCommitsFromBranch({ client, repo, branch, sinceIso, timeZone }) {
  const pages = [];
  for (let page = 1; page <= 5; page += 1) {
    const batch = await client.listCommits(repo, {
      sha: branch,
      since: sinceIso,
      perPage: 100,
      page,
  ...
  return pages.map((item) => {
    const commit = normalizeSearchCommit({
      sha: item.sha,
      commit: item.commit,
      author: item.author,
...
/**
 * Collect commits across main/develop + active feature branches in the lookback window.
 * (GitHub commit search mostly surfaces default-branch/QA history and misses [DEV] work.)
 */
async function fetchCommitsForDateRange({
  ...
  const sinceIso = lookbackStartIso(startKey);
  const keys = lookbackKeys?.length
    ? lookbackKeys
    : getLookbackDateKeys(timeZone, getLookbackDays());

  const branchNames = pickBranchesToScan(allBranches, defaultBranch, keys, timeZone);
  console.log(
    `[codebase-brief] scanning ${branchNames.length} branches for commits `
    + `${startKey}..${endKey} (since ${sinceIso})`,
  );

  const nested = await mapPool(branchNames, DETAIL_CONCURRENCY, async (branch) => {
    try {
      return await fetchCommitsFromBranch({ client, repo, branch, sinceIso, timeZone });
    } catch (error) {
      console.warn(`[codebase-brief] listCommits ${branch} failed: ${error.message}`);
      return [];
    }
  });

  const bySha = new Map();
  for (const commit of nested.flat()) {
    if (!commit.sha || !isCommitInLookback(commit, keys)) continue;
...
function formatBriefDataForPrompt(data) {
  const timeline = data.commits.length
    ? data.commits.map((commit) => {
      ...
      const branchNote = commit.branches?.length
        ? ` · [${commit.branches.slice(0, 3).join(', ')}${commit.branches.length > 3 ? ', …' : ''}]`
        : '';
      return `${day} ${time} | ${commit.shortSha}${branchNote} | ${commit.message}\n${statsLine}${paths}`;
    }).join('\n\n')
  ...
  return [
    ...
    'Commits: συλλογή από main/develop + όλα τα ενεργά feature branches στο lookback (όχι μόνο QA/main).',
...
function formatCommitDate(isoDate, timeZone) {
  if (!isoDate) return '—';
  return new Intl.DateTimeFormat('el-GR', {
    timeZone,
    day: '2-digit',
...
/** Markdown table embedded in the Word brief (rendered as a real table). */
function formatCommitsDocNote(data) {
  const {
    commits = [],
    lookbackDays,
    lookbackStart,
  ...
  for (const commit of commits) {
    const title = String(commit.message || '(no message)')
      .replace(/\|/g, '/')
      .replace(/\r?\n/g, ' ')
      .trim();
    const author = String(commit.author || 'unknown')
      .replace(/\|/g, '/')
      .trim();
    const date = formatCommitDate(commit.date, timeZone).replace(/\|/g, '/');
...
function formatCommitsMarkdown(data) {
  return formatCommitsDocNote(data);
}
...
/**
 * Append deterministic appendix sections into the AI day-plan brief (Word doc):
 * 1. Commits as a markdown table (rendered as a real Word table)
 * 2. Full stale-branch list
 */
function appendBriefAppendixSections(markdown, data) {
  let body = String(markdown || '').trim();

  const commitsSection = formatCommitsDocNote(data);
