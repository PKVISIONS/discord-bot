/**
 * Interactive GitHub issue creation — defaults to EmblemTameiaki, but callers
 * can pin another repo via repoFullName (e.g. /campaign-issues → campaign-studio).
...
async function startGitHubIssueFlow({
  ...
  const repoFullName = repoOverride || await resolveIssueRepo(repoHint);
  if (!repoFullName) {
    return {
