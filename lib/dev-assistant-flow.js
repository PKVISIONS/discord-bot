...
async function deliverDevResult(interaction, result, hubSession = null) {
  const attachment = buildDevMarkdownAttachment(result);

  if (hubSession) {
    await hubSession.sendMain({ content: result.content, components: [] });
    for (const extra of result.extraMessages || []) {
      await hubSession.sendFollowUp({ content: extra });
    }
    try {
      await hubSession.sendFollowUp({
      ...
      await hubSession.sendFollowUp('⚠️ Could not attach the markdown file. Check bridge logs.').catch(() => {});
    }
    return;
  }
