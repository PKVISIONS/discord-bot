 ...
 * Incremental: each chunk id is stable and skipped when its content hash is
 * unchanged; stale chunks for a source are pruned on a full namespace pass.
 */
...
const { buildVendorItems } = require('./vendor-assets');
...
async function reindexAll({ apiKey = process.env.OPENAI_API_KEY, onProgress = () => {} } = {}) {
  ...
  onProgress('Indexing vendor assets (PDF/AAR)…');
  results.push(await indexNamespace('vendor', await buildVendorItems(EMBLEM_REPO), { apiKey }));
