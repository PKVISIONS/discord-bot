...
- Ένα μόνο έγγραφο: γράψε μόνο το πλάνο ημέρας. Η πλήρης λίστα commits και stale branches προστίθενται αυτόματα.`;
...
function buildBriefUserPrompt(dataBlock, reportDateLabel) {
  return [
    ...
    'Έξοδος: ΕΝΑ markdown έγγραφο (όχι δύο briefs).',
    '',
    ...
    'ΜΗΝ γράψεις πλήρη λίστα κάθε commit — προστίθεται αυτόματα στο ίδιο έγγραφο.',
...
async function generateCodebaseBrief({
  ...
  // One file: AI day-plan + full commit list + full stale-branch list.
