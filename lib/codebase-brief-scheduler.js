 ...
 * Single Word attachment with day plan + commits table inside.
 */
...
async function deliverCodebaseBrief(discordClient, brief) {
  ...
  const summary = brief.commitCount
    ...
    : 'no commits';
  const staleNote = brief.staleBranchCount
    ? ` · ⚠️ ${brief.staleBranchCount} stale branch${brief.staleBranchCount === 1 ? '' : 'es'} (≥${brief.staleBranchDays || 7}d)`
    : '';

  await channel.send({
    content: [
      `📊 **BRIEF ΠΛΑΝΟΥ ΗΜΕΡΑΣ** — ${brief.reportDateLabel}`,
      `Repository: \`${brief.repoFullName}\` · lookback ${brief.lookbackDays || 3} ημέρες · ${summary}${staleNote}`,
      `📎 \`${brief.filename}\``,
    ].join('\n'),
    files: [attachment],
  });
