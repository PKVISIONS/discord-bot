...
function createClient(token, org) {
  ...
  return {
    ...
    async createIssue(repo, { title, body, labels = [], type = null }) {
      const payload = { title, body, labels };
      if (type) payload.type = type;
      return post(`/repos/${org}/${repo}/issues`, payload);
    },
    ...
    async listCommits(repo, { sha, perPage = 1, since, page = 1 } = {}) {
    ...
    async searchCommits(query, { perPage = 100, page = 1 } = {}) {
      ...
      return get(`/search/commits?${params}`);
    },
    ...
    async listIssues(repo, { state = 'open', perPage = 30, labels } = {}) {
      const params = new URLSearchParams({
        state,
        per_page: String(perPage),
        sort: 'updated',
      ...
      return get(`/repos/${org}/${repo}/issues?${params}`);
    },
  };
}
